...
var UIController = (function() {
	...
	var formatNumber = function(num, type) {
			...
			num = Math.abs(num);
			num = num.toFixed(2);//265.596 -> 265.60

			numSplit = num.split('.');// ["265" . "60"]

			int = numSplit[0];
			if(int.length > 3) {
				int.substr(0, int.length - 3) + ',' + int.substr(int.length - 3, 3); //256587
			}
	...
	return {
		...
		displayBudget: function(obj) {
			...
			document.querySelector(DOMstring.incomeLabel).textContent = formatNumber(obj.totalsInc, 'inc');
			document.querySelector(DOMstring.expensesLabel).textContent = formatNumber(obj.totalsExp, 'exp');
		...
		displayMonth: function() {
			...
			months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jon', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
